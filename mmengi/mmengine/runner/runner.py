...
from MB321.mm_flag import write_txt, agLOG, TM, SHOW_MANY, time_str
_log = [agLOG.bug_all]
if 'cls' in agLOG.task: _log = agLOG.cls + _log
if 'seg' in agLOG.task: _log = agLOG.seg + _log
write_txt([agLOG.bug_all], f" runner _log({len(_log)}) at:{_log}")
...
class Runner:
    ...
    def __init__(
        ...
        log_file = _log[0] #*mb321*
        write_txt(_log, f"{TM},op={cfg.op}, log_file: {osp.basename(log_file)}\n")
        self.logger = self.build_logger(log_level=log_level, log_file=log_file) #*mb321*
        ...
        write_txt(_log, f"Hooks will be executed in the following order:\n{self.get_hooks_info()}", b_prt=SHOW_MANY)
    ...
    def train(self) -> nn.Module:
        ...
        write_txt(_log, f"---start training @ {time_str()} ...")
    ...
    def val(self) -> dict:
        ...
        # make sure checkpoint-related hooks are triggered after `before_run`
        self.load_or_resume()
        write_txt(_log, f"---start validating @ {time_str()}...")
    ...
    def test(self) -> dict:
        ...
        # make sure checkpoint-related hooks are triggered after `before_run`
        self.load_or_resume()
        write_txt(_log, f"---start testing @ {time_str()}...")
    ...
    def _log_env(self, env_cfg: dict) -> None:
        ...
        if self.cfg._cfg_dict:
            write_txt(_log, f"\n self.cfg.pretty_text:\n {self.cfg.pretty_text}", b_prt=SHOW_MANY)
