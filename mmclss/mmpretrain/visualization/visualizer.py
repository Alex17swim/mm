...
class UniversalVisualizer(Visualizer):
    ...
    def visualize_cls(self,
        ...
        # input(f"draw_gt={draw_gt}; 'gt_label' in data_sample={'gt_label' in data_sample}")
        ...
        # input(f"draw_pred={draw_pred} 'pred_label' in data_sample={'pred_label' in data_sample}")
        ...
        # input(f"len={len(labels)},type={type(labels)}; v={labels};\n labels[0]({type(labels[0])})={labels[0]} -- _p{idx[0]}_{class_labels[0][2:-1]}.jpg")
        if out_file is not None:
            # save the image to the target file instead of vis_backends
            out_file = out_file[:out_file.rindex('.jpg')] + f"_p{idx[0]}_{class_labels[0][2:-1]}.jpg" #*mb321*
