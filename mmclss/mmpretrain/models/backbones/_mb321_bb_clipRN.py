# _mb321_bb_clipRN.py, Chengyu Wang, 2023-1120, last update:2023-1120
import sys, os
_import_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.append(_import_root)
# print("_mb321_bb_clipRN.py _import_root={}".format(_import_root))
...
def get_clip_net(bb_tag, num_cls, attn, cd, _prt=False):
    ...
    elif 'NEXT50' in bb_tag:
        net = LLM_next50(pretrained=False, num_classes=num_cls, attn=attn, cd=cd)
    else: input3(f"!!unexpect backbone tag:{bb_tag}({agNET.bb})"); net = None
...
if __name__ == '__main__':
    ...
    print(f"- main of _mb321_bb_clipRN.py done at {agBASE.tm}, from {time_str(tt0)}, cost {time_gap(tt0)}")
