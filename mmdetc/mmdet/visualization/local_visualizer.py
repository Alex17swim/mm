...
from MB321.mm_flag import agSK
clsT, clsF, b_sk_tag, show_gt, skip_f, sk_fac, fs = agSK.clsT, agSK.clsF, agSK.sk, agSK.show_gt, agSK.skip_f, agSK.fac, agSK.fs
...
# parser_sk.add_argument('--skip_f', default=False if B_SK_TAG else False, help="skip plot but still count instance number")
...
class DetLocalVisualizer(Visualizer):
    ...
    def _draw_instances(self, image: np.ndarray, instances: ['InstanceData'],
        ...
        # input(f" bboxes={'bboxes' in instances and instances.bboxes.sum() > 0}; mask={'masks' in instances}\n")
        if not b_sk_tag: # 'bboxes' in instances and instances.bboxes.sum() > 0:
            # if b_sk_tag: input(f"... you make need only segm instead of bbox for metric in config file\n\n")
        ...
        if 'masks' in instances:
            ...
            # print(f" instances labels({len(labels)},{type(labels)})={labels}; masks({len(masks)},{type(labels)})={masks}")
            ...
            polygons, sk, m2 = [], {'T':[], 'F':[], 'TF':[]}, np.ndarray # instances.masks
            for i, mask in enumerate(masks):
                contours, _ = bitmap_to_polygon(mask)

                ss, l0, ll = 0, len(contours[0]), len(contours) # , size={np.size(contours)}
                for ii in range(0, ll): ss += int(contours[ii].size/2) # len(contours[0])
                # print(f"i={i}, lb={labels[i]}, l0={l0}, ll={ll}; ss={ss}; mask({len(mask)})") # _poly = '' if l0 > 10 else f"\n\npoly({l0})={contours[0]}, type{type(contours[0])}"
                sk['TF'].append(ss * sk_fac)

                if int(labels[i]) == clsF: sk['F'].append(ss * sk_fac)
                else: sk['T'].append(ss * sk_fac)
                if skip_f and int(labels[i]) == clsF: #  and len(labels) > 0 # in case NO labels
                    # print(f"skip poly: poly={len(polygons)}; labels({len(labels)})={labels[i]}")
                    continue
                polygons.extend(contours)
                # np.stack(m2, mask)
                # input(f"m2({len(m2)}, {type(m2)})={m2}")
            # end for i, mask in
            # if len(classes) > 1 and skip_f and (clsF in labels):
            #     # _id = np.where(labels == 0)
            #     _id = np.array([int(k) for k in labels if int(k) == clsT])
            #     input(f"{len(classes)}, {labels}, _id({len(_id), type(_id)}){_id}, masks({len(masks)}), {type(masks)})={masks}")
            #     masks2 = [kk[ii] for kk in masks for ii in _id]
            #     input(f"masks2 1({len(masks2)}, {type(masks2)})={masks2}")
            #     masks2 = np.ndarray(masks2[0])
            #     input(f"masks2 2({len(masks2)}, {type(masks2)})={masks2}")
            # else: masks2=masks
            self.draw_polygons(polygons, edge_colors='w', alpha=self.alpha)
            self.draw_binary_masks(masks, colors=colors, alphas=self.alpha)

            if len(labels) > 0 and (('bboxes' not in instances or instances.bboxes.sum() == 0) or b_sk_tag):
                ...
                areas = np.stack(areas, axis=0) # type=numpy.ndarray
                scales = _get_adaptive_scales(areas)

                j = 0
                for i, (pos, label) in enumerate(zip(positions, labels)):
                    if skip_f: # *mb321*
                        if int(label) == clsF: # _lb_txt = ''
                            print(f"skip label={label}")
                            continue
                        else: _lb_txt = f"{sk['T'][j]}"
                    else: _lb_txt = f"{sk['TF'][j]}" # both T and F
                    if 'label_names' in instances:
                        label_text = instances.label_names[i]
                    else:
                        # if label >= len(classes): #*mb321* still plot polygons mask
                        #     print(f"label={label}, len={len(classes)}")
                        #     continue
                        label_text = classes[label] if classes is not None else f'class {label}'
                    label_text += f":{_lb_txt }" # *mb321*
                    # if 'scores' in instance s: # *mb321*
                    #     score = round(float(instances.scores[i]) * 100, 1)
                    #     label_text += f': {score}'
                    ...
                    j += 1  # *mb321* for only show T label
                # end for i, (pos, label)
            self.sk1 = f"T{len(sk['T'])}={sum(sk['T'])}"
            self.sk0 = f"F{len(sk['F'])}={sum(sk['F'])}"
            self.draw_texts(
                f"{self.sk1}\n{self.sk0}" if len(classes) > 1 else f"{self.sk1}",
                np.full(2, [200.0, 50.0]),
                colors=get_palette(self.text_color, 1), # text_palette[0], # text_colors[0],
                font_sizes=32,
                horizontal_alignments='center',
    ...
    def _draw_sem_seg(self, image: np.ndarray, sem_seg: PixelData,
        ...
        Returns:
            np.ndarray: the drawn image which channel is RGB.
        """
        print(f"!!*mb321* semantic only, NOT instance!\n")
    ...
    def add_datasample(
        ...
        if draw_pred and data_sample is not None:
            ...
            if 'pred_sem_seg' in data_sample: # semantic only, NOT instance
        ...
        if show_gt:
            ...
            mmcv.imwrite(gt_img_data[..., ::-1], out_file.replace('.jpg', '_gt.jpg'))
        self.set_image(pred_img_data)
        mmcv.imwrite(pred_img_data[..., ::-1], out_file.replace('.jpg', f'_pred{self.sk1}.jpg'))
