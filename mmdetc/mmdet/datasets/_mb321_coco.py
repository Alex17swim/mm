...
class MB321coco(BaseDetDataset): # Dataset for COCO format: ('chr',) or ('T', 'F', ) 
    write_txt([agLOG.bug_all], f"## MB321coco METAINFO: agMM.classes_seg={agMM.classes_seg}")
    METAINFO = {
        'classes': agMM.classes_seg,
    ...
    def load_data_list(self) -> List[dict]:
        ...
        if self.ANN_ID_UNIQUE: assert len(set(total_ann_ids)) == len(total_ann_ids), f"Annotation ids in '{self.ann_file}' are not unique!"
