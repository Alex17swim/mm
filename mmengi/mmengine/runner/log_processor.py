...
from MB321.mm_flag import EPOCH_C, EPOCH_S, agBASE
epoch_max = EPOCH_S if 'seg' in agBASE.task else EPOCH_C
        ...
        if self.by_epoch:
            ...
            if mode in ['train', 'val']:
                ...
                if not (isinstance(runner._train_loop, dict)
                    ...
                    # in " [{max_epochs}]"
                    cur_epoch_str = f'[{cur_epoch}/{epoch_max}]'.rjust(
                        len(str(max_epochs)) + 3, ' ')
                else:
                    cur_epoch_str = f'[{cur_epoch}/{epoch_max}]'
                tag['epoch'] = cur_epoch
